  ...
  const downloadReport = () => {
    if (!analysisResult) {
      toast({
        ...
        description: "Please analyze a file first to generate a report",
        variant: "destructive"
      });
      return;
    }
    ...
    const reportContent = `
SATYAAI DEEPFAKE DETECTION REPORT
==========================================

Analysis Date: ${new Date(analysisResult.analysis_date).toLocaleString()}
Case ID: ${analysisResult.case_id}
Processing Time: ${analysisResult.processing_time}
File Name: ${selectedFile?.name || 'Unknown'}
File Size: ${selectedFile ? (selectedFile.size / 1024 / 1024).toFixed(2) + ' MB' : 'Unknown'}

AUTHENTICITY ASSESSMENT
==========================================
Status: ${analysisResult.authenticity}
Confidence Level: ${analysisResult.confidence_percentage}%
Forensic Score: ${analysisResult.forensic_score}/100
Risk Level: ${analysisResult.risk_level || 'MEDIUM'}

KEY FINDINGS
==========================================
${analysisResult.key_findings.map(finding => `• ${finding}`).join('\n')}

DETAILED TECHNICAL ANALYSIS
==========================================
Multimodal Assessment: ${analysisResult.detailed_analysis.multimodal_assessment}
Neural Network Analysis: ${analysisResult.detailed_analysis.neural_network_analysis}
Quantum Processing: ${analysisResult.detailed_analysis.quantum_processing}
Forensic Validation: ${analysisResult.detailed_analysis.forensic_validation}
Biometric Verification: ${analysisResult.detailed_analysis.biometric_verification}
Temporal Consistency: ${analysisResult.detailed_analysis.temporal_consistency}
Spectral Analysis: ${analysisResult.detailed_analysis.spectral_analysis}
Metadata Forensics: ${analysisResult.detailed_analysis.metadata_forensics}
AI Generation Detection: ${analysisResult.detailed_analysis.ai_generation_detection}
Deepfake Signatures: ${analysisResult.detailed_analysis.deepfake_signatures}

BLOCKCHAIN VERIFICATION
==========================================
Hash: ${analysisResult.blockchain_verification?.hash || 'N/A'}
Timestamp: ${analysisResult.blockchain_verification?.timestamp || 'N/A'}
Integrity Proof: ${analysisResult.blockchain_verification?.integrity_proof || 'N/A'}

COMPLIANCE STATUS
==========================================
GDPR Compliant: ${analysisResult.compliance_status?.gdpr_compliant ? 'Yes' : 'No'}
HIPAA Compliant: ${analysisResult.compliance_status?.hipaa_compliant ? 'Yes' : 'No'}
ISO27001 Aligned: ${analysisResult.compliance_status?.iso27001_aligned ? 'Yes' : 'No'}
NIST Framework: ${analysisResult.compliance_status?.nist_framework || 'N/A'}

RECOMMENDATION
==========================================
${analysisResult.recommendation}

Report generated by SatyaAI Cyber Intelligence Platform
© 2024 SatyaAI. All rights reserved.
    `;
    ...
    link.href = url;
    link.download = `SatyaAI_Analysis_Report_${analysisResult.case_id}.txt`;
    ...
    toast({
      ...
      description: "Analysis report has been saved to your downloads",
    });
  };
